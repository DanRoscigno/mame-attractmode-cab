...
function hiscore.startplugin()
	...
	local config_path = "/home/rastan/.mame/hiscore.ini";
	emu.print_verbose( "Dan: " .. config_path );
	...
	local function read_config()
	  if config_read then return true end;
          emu.print_verbose( "Dan: in read_config(), attempting open of " .. config_path );
	  local file = io.open( config_path, "r" );
	  if file then
                emu.print_verbose( "Dan: in read_config(), file returned true");
		file:close()
		emu.print_verbose( "hiscore: config found" );
		local _conf = {}
		for line in io.lines(config_path) do
                  emu.print_verbose( "Dan: looping over config " .. config_path );
                  emu.print_verbose( "Dan: line is " .. line );
		  -- local token, value = string.match(line, '([^ ]+) ([^ ]+)');
		  -- local token, value = string.match(line, '(%g+) (%g+)');
		  _conf['hi_path'] = '/home/rastan/.mame/hi';
		end
